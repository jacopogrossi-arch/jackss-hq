# Content Calendar

Tracker dei contenuti: idee → in lavorazione → pubblicato

---

## In lavorazione

| # | Titolo | Formato | Stato | Note |
|---|--------|---------|-------|------|
| — | *(nessun video in lavorazione — prossimo dal backlog)* | | | |

---

## Pubblicati

| # | Titolo | Piattaforma | Data | Link | Note |
|---|--------|-------------|------|------|------|
| 1 | What if AI could build a fashion brand from scratch? (Manifesto) | YouTube + Instagram | 02/06/2026 | — | Stack: HiggsField Soul V2 → Kling 3.0 + Veo → HiggsField voice → CapCut. IG 75 view / YT 34 view (al 10/06) |
| 2 | The Jago Brand — Built in One Afternoon | YouTube + Instagram | 07/06/2026 | — | Reel ~34s. Proof of concept del Brand Identity Pack. IG 123 view / YT 8 view (al 10/06) |
| 3 | A Fashion Lookbook — Built with AI | YouTube + Instagram | 12/06/2026 | — | Reel/Short ~48s, inglese. Replica campagna Stone Island Marina SS26 con Jago. CTA "LOOKBOOK" → lead magnet via Beehiiv (funnel attivo, link in descrizione YT). Outreach DM partito (`outreach.md` + routine daily) |

---

## Backlog — Prossimi video

| # | Titolo | Formato | Stato |
|---|--------|---------|-------|
| 4 | Come ho generato un SVG per cartamodelli usando solo Claude | YouTube 8min + clip | idea |
| 5 | Kling per fashion video: quello che nessuno ti dice | YouTube 8min + clip | idea |
| 6 | Da concept a campagna: flusso completo AI per un lookbook | YouTube 10min + clip | idea |
| 7 | Come uso Claude per la modellistica e i pattern | YouTube 8min | idea |
| 8 | Budget reale di un brand di moda fatto con AI | YouTube 10min | idea |
| 9 | I tool che uso ogni giorno come studente fashion design + AI | YouTube 8min + clip | idea |
| 10 | Errori che ho fatto usando AI nella moda | YouTube 10min | idea |
| 11 | Come creo campagne luxury con Gemini e HiggsField | YouTube 8min + clip | idea |
| 12 | Il mio sistema completo: 6 mesi di AI applicata alla moda — recap | YouTube 15min | idea |

---

## Metriche — Aggiornamento mensile

| Mese | Video pubblicati | IG followers | YouTube subs | Email list | Clienti paganti | Entrate |
|------|-----------------|--------------|--------------|------------|-----------------|---------|
| Giugno 2026 (al 10/06) | 2 | 2 | 0 | 0 | 0 | €0 |
| Luglio 2026 | — | — | — | — | — | € — |
| Agosto 2026 | — | — | — | — | — | € — |
