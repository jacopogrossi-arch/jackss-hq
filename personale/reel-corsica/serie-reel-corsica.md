# Serie reel Corsica — 40 clip, 6 idee

Materiale: ~40 clip cinematografiche, statiche, color gradate.
Pubblico: amici e famiglia. Registro: diario, non travel content.

**40 clip non sono 40 reel.** Il limite non e' il materiale, e' la pazienza di chi guarda.

## INVENTARIO REALE (verificato)

- **~8 clip con persone**, di cui **4-5 di spalle** (Jacopo o un amico)
- **Poche B**: acqua, e soprattutto **riprese dalla barca** — le uniche con movimento vero
- **Il resto e' C**: paesaggio bello e immobile, ~25 clip

**Conseguenza: tre reel, non sei.** Con 3-4 clip di volti veri, un reel di sole persone
non sta in piedi. Tre reel fatti bene battono sei che riciclano lo stesso paesaggio.

**Conseguenza 2: le clip in barca sono il materiale migliore, non il ripiego.**
Con 25 clip immobili, la barca e' l'unica cosa che si muove da sola: orizzonte che scorre,
acqua, persone dentro. E' la spina dorsale di un reel, non riempitivo.

---

## FASE 0 — Smistamento (da fare prima di tutto)

Cinque cartelle. Serve mezz'ora e rende ogni decisione dopo automatica.

| Mucchio | Cosa | Note |
|---|---|---|
| **A** | Con persone | volti, mani, schiene, qualcuno che cammina |
| **B** | Statiche ma vive | acqua, vento, tende, fumo, fuoco, foglie |
| **C** | Paesaggio puro | bello e immobile |
| **D** | Dettagli | cibo, mappa, piedi, oggetti, texture |
| **E** | Imperfette | mosse, storte, sbagliate — **non buttarle** |

Aspettativa realistica: troppa C, pochissima A. E' il problema tipico del materiale cinematografico.
Il numero di clip in **A** decide quanti reel puoi fare davvero.

---

## La grammatica del montaggio

Cinque regole. Valgono per tutti i reel della serie.

1. **Mai due clip di paesaggio puro consecutive.**
   Due C di fila e il reel diventa uno screensaver. Spezza sempre con A, B o D.

2. **Una persona ogni tre clip.**
   E' la regola che tiene sveglio chi guarda. Anche solo una mano o una schiena.

3. **Alterna la scala.**
   Largo → dettaglio → medio → largo. Mai due inquadrature della stessa ampiezza di fila.
   E' quello che crea ritmo quando le clip sono tutte ferme.

4. **Se la clip e' statica, dentro deve muoversi qualcosa.**
   Acqua, vento, una persona che entra nel frame. Una cosa ferma ripresa ferma e' una foto,
   e le foto si scorrono. Le clip del mucchio C senza movimento interno vanno in fondo o fuori.

5. **Durate: larghe 1,5-2s · dettagli 0,8-1,2s.**
   Le clip belle ti tentano a tenerle quattro secondi. Non farlo — tranne la prima e l'ultima,
   che possono respirare fino a 3s.
   **Taglia sul respiro della musica, non su ogni beat.** Il taglio a ritmo serrato e' la firma
   dell'edit da travel influencer.

**Conto pratico:** un reel di 28s con queste durate sono ~15 clip. Sei reel non consumano 40 clip
senza ripetizioni: mettere in conto che alcune clip forti tornino in due reti diversi va benissimo,
purche' non siano nella stessa posizione.

---

## Calendario

Uno ogni 4-5 giorni. Tre reel = circa due settimane di racconto.
Il viaggio dura di piu' e il pubblico non satura.

**I tre da fare:** 1 (l'arco) · 2 (la barca) · 3 (la giornata storta, se e' successa davvero).
I reel 4, 5 e 6 restano qui sotto come riserva: con questo inventario riciclerebbero
le stesse clip, quindi falli solo se i primi tre vanno bene e ti avanza voglia.

---

# LE SEI IDEE

## 1. "Non volevo partire" — l'arco del viaggio
**Il pezzo principale, esce per primo.** Hook e caption in `hook-reel-corsica.md`.

### Il principio

Le clip sono tutte belle e ferme allo stesso modo: il cambiamento non puo' venire
dal loro contenuto. Viene dalla loro **qualita'**.

**Il reel diventa progressivamente piu' vivo.** Si parte dalle clip piu' immobili e vuote,
si finisce con le piu' mosse e piene. Cosi' le ~25 clip di paesaggio fermo hanno un lavoro
vero: sono il "prima", e la loro immobilita' e' voluta.

### ATTO 1 — la stasi (0–9s)

| Tempo | Clip | Testo |
|---|---|---|
| 0 – 3s | **tu di spalle**, fermo, piccolo nel frame, vuoto intorno | `Non volevo partire.` |
| 3 – 5s | **C** vuota | |
| 5 – 7s | **C** vuota | |
| 7 – 9s | **D** dettaglio freddo: chiavi, valigia, un oggetto | |

**Audio:** ambientale reale 0–2,5s, poi entra la musica. Non di piu':
otto secondi di solo ambiente non danno atmosfera, danno la sensazione che il video
non sia partito, e l'hook resta senza ritmo sotto.

**L'apertura risolve due esigenze in una.** Una persona ferma e piccola dentro
un'inquadratura vuota e' contemporaneamente un essere umano (che tiene chi guarda)
e la stasi che serve all'arco. Quella clip *e'* "non volevo partire".

**La faccia al secondo 0 no, e non per gusto: per aritmetica.** Ne hai 3-4 in tutto.
Spesa all'inizio non ha niente su cui atterrare e lascia l'atto 3 senza il suo pagamento.
Le facce sono la prova che qualcosa e' cambiato: vanno dove il cambiamento e' gia' avvenuto.

**Qui la regola "mai due C di fila" e' sospesa.** Tre paesaggi consecutivi e immobili sono
lo strumento: creano la stasi che l'atto 2 rompe. Unico punto del piano dove la noia e' voluta.
Clip lunghe, 2-3 secondi.

### ATTO 2 — si muove qualcosa (9–18s) · clip corte, 1–1,5s

| Tempo | Clip |
|---|---|
| 9 – 11s | **la clip piu' cineticamente forte che hai** — la prua che taglia l'onda, un tuffo, la scia. E' lo stacco del reel |
| 11 – 12,5s | **B** acqua o barca |
| 12,5 – 13,5s | **C** corta (1s) |
| 13,5 – 15s | **B** acqua |
| 15 – 16s | **D** dettaglio |
| 16 – 18s | **B** barca, movimento |

### ATTO 3 — vivo (18–27s) · clip di nuovo lunghe, 2s

| Tempo | Clip | Testo |
|---|---|---|
| 18 – 20s | la clip con la **faccia** migliore, tenuta lunga | |
| 20 – 21,5s | movimento: un tuffo, acqua | |
| 21,5 – 23s | seconda faccia, o il gruppo | |
| 23 – 25s | **la clip piu' bella del viaggio**, larga | |
| 25 – 27s | **mucchio E** — la clip mossa, storta, brutta | `Poi e' andata cosi'.` |

### Le tre cose che fanno la differenza

**Non aprire con la clip piu' bella.** E' l'istinto naturale ed e' l'errore piu' grosso:
se parti dal meglio, tutto il resto e' discesa e l'arco non esiste. La clip migliore va
al secondo 23, dove diventa la prova che qualcosa e' cambiato.

**Ritmo: lento → veloce → lento-ma-pieno.** E' la curva che fa sentire il cambiamento
anche a chi non ci sta facendo caso. Le durate qui sopra la costruiscono da sole.

**La svolta la porta la canzone, non il silenzio.** Scegli un pezzo con l'intro rada
che poi cresce, e allinea il punto in cui il brano **si apre** al secondo 9, sul cambio d'atto.
Ogni ingresso o cambio di musica cade su uno stacco, mai a meta' clip.

### Chi guarda in muto

Molta gente guarda i reel senza audio. Per loro **l'ingresso della musica al 2,5s e la sua
apertura al 9s non esistono**: tutta la svolta sonora e' invisibile.

Il design regge lo stesso, perche' l'arco e' prima di tutto visivo (fermo → in movimento).
Ma ne segue una regola precisa: **la clip al secondo 9 deve essere la piu' cinetica
dell'archivio**, non genericamente "acqua". E' l'unico segnale del cambio d'atto per chi
guarda in silenzio, quindi da sola deve far capire che il video e' cambiato.

### Rifiniture

- **Grading come narrazione:** atto 1 piu' freddo (-10 saturazione, temperatura verso il blu),
  atto 3 piu' caldo. Due secondi per clip in CapCut, raddoppia l'effetto dell'arco.
  Senza esagerare: deve restare lo stesso viaggio.
- **Testo:** piccolo, bianco, senza ombra pesante, in basso. Il font grande e grassetto
  e' la firma dell'influencer, cioe' il registro che stiamo evitando.
- **Solo due testi in tutto il reel.** Nessuna scritta tra il secondo 3 e il 25.

---

## 2. La barca
**Sostituisce "Le persone" (tagliato: 4-5 clip su 8 sono di spalle).**
**Costruito sull'unico materiale con movimento vero.**

```
0 – 2,5s   la prua che taglia l'acqua — SOLO audio reale del motore, niente musica
           TESTO: "L'unico giorno in cui non ho pensato a niente."
2,5s       entra la musica
2,5 – 5s   qualcuno di spalle seduto a prua  (il di-spalle qui funziona: si muove tutto intorno)
5 – 7s     dettaglio: mano nell'acqua, una corda, piedi bagnati
7 – 12s    4 clip di acqua e costa dalla barca — corte, 1,2s l'una
12 – 15s   l'unica clip con una faccia, tenuta lunga
15 – 18s   un tuffo, o l'acqua ripresa dall'alto
18 – 21s   la costa che si allontana, clip lunga
```

**Perche' funziona:** e' l'unico reel in cui il movimento e' nelle immagini e non nel montaggio.
Puoi tenere i tagli lenti senza che sembri fermo.
**Suono:** ambientale reale per 2,5s, poi la canzone del viaggio.
**Caption:** un dettaglio concreto della giornata. Niente riassunti.

---

## 3. La giornata storta
**Il candidato per il suono di tendenza. Il piu' condivisibile.**

Un solo giorno, quello andato male. Il motore comico e' il contrasto:
immagini perfette, testo che racconta il disastro. Il testo cambia a ogni clip.

```
0 – 2s     clip bellissima, alba o partenza
           TESTO: "9:00 — partiamo carichi"
2 – 5s     clip bellissima     "11:00 — sbagliamo strada"
5 – 8s     clip bellissima     "13:00 — finisce la benzina"
8 – 11s    clip bellissima     "15:00 — [disastro reale]"
11 – 14s   clip bellissima     "18:00 — [disastro reale]"
14 – 18s   la clip PIU' bella  "21:00 — la foto piu' bella del viaggio"
```

Le ore e i disastri vanno sostituiti con quelli veri. La struttura regge solo se i fatti sono veri:
inventati, si sente subito perche' il ritmo diventa troppo pulito.

**Il ritmo e' tutto:** ogni clip dura uguale, tagli netti, nessuna dissolvenza. E' una lista, non un montaggio.
**Suono:** qui il trending funziona — ironico, o un pezzo epico serissimo usato in modo sproporzionato.
**Caption:** una riga sola. `Comunque rifarei tutto tranne la benzina.`

---

## 4. Un posto solo
**Il piu' lento e il piu' bello. Zero pretese virali.**

6-8 clip dello stesso luogo, ordinate per ora del giorno. La luce che cambia e' l'unica narrazione.

```
0 – 3s     il posto, luce del mattino
           TESTO: "Ci sono tornato tre volte in sei giorni."
3 – 20s    5-6 clip, stesso posto, luce sempre diversa
           tenute lunghe (2,5-3s), in ordine cronologico di luce
20 – 23s   il posto di notte o al tramonto, la piu' forte
```

**Deroga alla regola 1:** qui le clip di paesaggio consecutive sono ammesse, perche' il soggetto
e' identico e il cambio di luce fa da movimento. E' l'unico reel dove funziona.
**Suono:** ambientale reale per 3 secondi, poi cantautorato italiano lento. Niente ambient da libreria.
**Caption:** perche' ci sei tornato. Una frase, concreta.

---

## 5. Primo giorno / ultimo giorno
**Il piu' semplice da montare. Mezz'ora di lavoro.**

```
0 – 2s     TESTO grande: "Primo giorno"
0 – 10s    5-6 clip girate il giorno 1 — includi le piu' goffe
10 – 11s   stacco netto, un frame nero o un taglio su una clip scura
           TESTO grande: "Ultimo giorno"
11 – 22s   5-6 clip del giorno 6 — le piu' belle, le piu' sciolte
```

Funziona perche' la differenza si vede da sola: le prime clip sono sempre piu' rigide,
le ultime piu' vive. Non serve commentarlo.
**Suono:** una canzone sola che cresce, lo stacco cade sul cambio del pezzo.
**Caption:** `Stessa isola, sei giorni di differenza.`

---

## 6. Il dump muto
**Da tenere per ultimo, come chiusura della serie.**

Nessun testo, nessun hook, nessuna narrazione. Le 12 clip piu' belle e la canzone.
Ordine: alterna scala e mucchi, chiudi sulla clip piu' umana che ti resta.

Funziona **solo come ultimo della serie**: dopo cinque reel raccontati, uno muto legge come congedo.
Pubblicato per primo, sarebbe solo un bel niente.

**Caption:** solo `Corsica.` e il tag localita'.

---

## Le clip di spalle: come usarle

Non sono inutili, sono mal posizionate se finiscono in mezzo.

- **Massimo due per reel**, distanti tra loro. Tre diventano un tic e si nota.
- **In apertura o chiusura**, mai nel corpo centrale.
- **Solo dove si muove qualcos'altro nel frame**: sulla barca, con le onde, col vento nei vestiti.
  Una schiena ferma davanti a un panorama fermo e' la definizione di foto stock.
  Una schiena che **entra** o **esce** dall'inquadratura e' viva.

---

## Salvare le clip C (ne hai ~25, vanno usate)

**Accorciale.** Una clip statica di 2s e' uno stallo; la stessa a 0,8-1s e' un battito.
Controintuitivo con materiale bello, ma le clip di paesaggio vanno **piu' corte** delle altre,
non piu' lunghe. Questa deroga sostituisce la regola 5 per il mucchio C.

**Punch-in impercettibile.** Scala da 100% a 104% sulla durata della clip
(CapCut: zoom con due keyframe). Aggiunge movimento dove non ce n'e'.
**Una clip su tre al massimo** — applicato a tutte si vede e sembra un preset.

**Il format che le salva davvero e' il reel 3.** Quando e' il testo a raccontare,
la clip bella e immobile fa da spalla alla battuta e non serve nessuna persona dentro.

---

# SUONI

## Quello che non posso darti

Non ho accesso ai trend audio in tempo reale, quindi **non trovi qui una lista di "suoni virali
di agosto 2026"**: sarebbe inventata o vecchia, e ti farebbe perdere tempo.

## Come trovarli in 5 minuti

1. Instagram → crea reel → icona musica → sezione **In tendenza**
2. Cerca la **freccina ↗** accanto al nome: e' un suono in crescita
3. Guarda il **numero di reel** che lo usano:
   - sotto ~50.000 → in salita, e' il momento giusto
   - sopra ~1.000.000 → tardi, sei l'ultimo ad arrivare
4. Salvane 10 nei preferiti mentre scrolli, cosi' li hai pronti quando monti

## Il punto vero

**Sulle clip cinematografiche lente il suono trending e' spesso il nemico.**
I suoni di tendenza sono veloci, vocali, spesso meme. Su un paesaggio immobile fanno a pugni
e leggono come "audio messo a caso".

**Regola per questa serie: un solo reel cavalca un suono di tendenza — il numero 3.**
Tutti gli altri usano musica vera.

## Registri che funzionano, in ordine

1. **La canzone del viaggio** — quella ascoltata davvero li'. Sempre la prima scelta.
   Per chi c'era e' un innesco di memoria, per chi non c'era e' comunque una scelta tua.
2. **Cantautorato italiano lento** — effetto enorme su un pubblico italiano, perche' il testo
   lo capiscono e diventa memoria condivisa. Scegli il pezzo meno ovvio dell'album.
3. **Audio ambientale reale** — vento, mare, voci vostre.
4. **Ambient / piano da libreria** — sicuro ma anonimo. Rischio concreto: sembra video di stock.

## Il trucco che vale piu' della canzone

**Lascia 2 secondi di audio ambientale vero all'inizio, poi fai entrare la musica.**

Vento, mare, una voce, il motore della macchina. Costa niente e trasforma il reel
da montaggio a documento. Vale per tutti e sei.
