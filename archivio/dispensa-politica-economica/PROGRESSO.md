...
**Esito del ripasso leggero di stasera**: `ripasso-esercizi.html` riadattato con tab di default "I 5 core (A1→A13)" (vedi commit). Jacopo ha ripassato **A1→A4**, tutti confermati bene. Unico intoppo: **A4** (sostenibilità del debito) — non si ricordava la formula `Δ(B/Y) ≈ (i−ṗ−Ẏ)·(B/Y) − avanzo primario/Y` a memoria, ma una volta rivista ha ricostruito tutto senza problemi. È l'unica formula del blocco 1 che va richiamata a memoria invece che ricostruita passo passo come i moltiplicatori — da ridare una lettura veloce domani mattina, non è un punto debole strutturale. Fermato qui per stasera su richiesta di Jacopo (riposo prima dello sprint di domani). A5→A13 non ancora ripassati stasera.

**Nota di fiducia esplicita di Jacopo**: si sente sicuro di **A1-A3** (moltiplicatori: economia chiusa, imposta proporzionale, economia aperta) "in qualsiasi condizione", cioè anche a freddo/sotto pressione — non solo dopo averli riletti. Per lo sprint di domani questo sposta il tempo: **A1-A3 non hanno bisogno di altro ripasso**, il tempo va concentrato su A4 (solo il richiamo della formula), A5-A8 (mai ritestati a freddo) e soprattutto A6/A9/A10/A12/A13 (i punti con errore o debolezza già noti).

## Lunedì 7/9 sera — Giorno 5, sprint pre-esame (esame domani mattina)

Sessione partita da zero (nuova chat, senza contesto del piano) con "non penso di sapere abbastanza per l'esame di domani". Ricostruito il contesto leggendo README+mappa priorità, poi git pull ha portato alla luce PROGRESSO.md e il giorno-4: da lì recuperato il piano reale (sprint sui 5 core, tre punti deboli aperti).

**Prima parte (senza sapere ancora del piano)**: interrogazione a quiz sui 4 argomenti core con numeri nuovi (non dal quaderno) — tutti risolti bene con al più un paio di piccoli errori di distrazione (divisione invertita nel monopolio, etichette scambiate tra tasso di attività/occupazione nel mercato del lavoro, entrambi autocorretti dopo spiegazione). Toccati anche esternalità e cenni R4/R13 (fallimenti dello Stato, VAN/TIR) a livello di framework.

**Seconda parte (dopo il git pull, sui punti deboli reali del quaderno)** — **i tre punti deboli storici sono stati chiusi tutti e tre stasera:**

| Esercizio | Esito | Note |
|---|---|---|
| **A6(c)** — Phillips "al contrario" (obiettivo π=8%, trova u) | ✅ **risolto con un hint, poi A6(c)-variante (π=6%) risolto completamente da solo** | Il blocco storico era saltare da π a u senza passare per ẇ. Sbloccato dando la procedura esplicita a due passi (isola ẇ = π+θ̇, poi sostituisci nella curva dei salari). Alla seconda prova (obiettivo π=6%→u=8,5%) l'ha fatto interamente da solo, incluso l'ultimo passaggio di divisione. **Punto debole chiuso.** |
| **A12** — obiettivo-strumento (θ=25,c=0,75,I=2000,N=1600) | ✅ **tutti i punti (a,b,c) risolti da solo**, un piccolo inciampo sulla formula dell'efficacia | (a) G=8.000 (t=0) e (b) G=14.000 (t=0,2) calcolati correttamente, un numero alla volta, senza mai passare dalle lettere. (c) primo tentativo ha dato il denominatore (6,25) invece dell'efficacia (1/6,25=0,16) — errore concettuale minore, corretto subito capendo la differenza fra "costo per occupato" ed "efficacia". **Il metodo "solo numeri, mai lettere" (isolato il 4/9) ha funzionato di nuovo, confermando che è la strategia giusta per Jacopo su questo genere di esercizio.** |
| **A13** — sistema Tinbergen (Y=2G+e, CC=−0,4G+0,8e, Y*=500,CC*=0) | ✅ **risolto quasi interamente da solo** | Primo impatto "non ho capito che devo fare" (stesso tipo di blocco iniziale di A12 il 4/9), sbloccato scomponendo la sostituzione in passaggi minimi (isola G da un'equazione, sostituisci, risolvi). Da lì in poi tutti i passaggi numerici corretti al primo colpo: G=200, e=100, verifica fatta. Ha poi enunciato **da solo e correttamente** la regola aurea di Tinbergen nella sua interezza (sovra/esatto/sotto-determinato) senza che gliela riscrivessi. **Punto debole chiuso, incluso il corollario teorico.** |

**Pattern confermato stasera**: il metodo che funziona su Jacopo per gli esercizi "a più pezzi" è sempre lo stesso — **scomporre in passaggi numerici minimi, mai una formula intera in un colpo solo, mai partire dalle lettere**. Ha funzionato identico su tre esercizi di natura diversa (Phillips inverso, Tinbergen numerico, sistema lineare 2x2). È una conferma forte, non solo un'ipotesi: vale la pena riproporlo per qualunque nuovo tipo di esercizio incontrato a ridosso dell'esame.

**Stato alla vigilia dell'esame (mattina 8/9)**: tutti e 3 i punti deboli storici del quaderno A1-A13 sono chiusi. Nessun punto debole aperto sui 5 core. Se resta tempo domattina prima dell'esame: rilettura veloce della formula di sostenibilità del debito (A4, l'unica da richiamare a memoria invece che ricostruire) e un ripasso a lettura (non a esercizio, per non caricare ansia) dei 5 argomenti secondari già validati al 9/11 nei quiz di preselezione.
